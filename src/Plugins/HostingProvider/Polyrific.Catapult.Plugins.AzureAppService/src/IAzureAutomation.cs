...
namespace Polyrific.Catapult.Plugins.AzureAppService
{
    public interface IAzureAutomation
    {
        ...
        /// <param name="isAllowRename">Allows renaming the app service if the original app service name is not available?</param>
        /// <returns>Host location and Error message if any</returns>
        Task<(string hostLocation, string error)> DeployWebsite(string artifactLocation, string subscriptionId, string resourceGroupName, string appServiceName, string deploymentSlot, string connectionString, string regionName, string planName, bool isAllowRename);
    }
}

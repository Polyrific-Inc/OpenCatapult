...
namespace Polyrific.Catapult.Engine.Core.JobTasks
{
    ...
    {
        /// <inheritdoc />
        public TestTask(IProjectService projectService, IExternalServiceService externalServiceService, IExternalServiceTypeService externalServiceTypeService, IPluginService pluginService, IPluginManager pluginManager, ILogger<TestTask> logger)
        ...
        public override async Task<TaskRunnerResult> RunMainTask(Dictionary<string, string> previousTasksOutputValues)
        {
            ...
            if (result.ContainsKey("testResultLocation"))
                returnValue = $"The test result is available in: {result["testResultLocation"].ToString()}";
